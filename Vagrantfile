...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", env: {'APT_SOURCE' => options['apt_source']}, inline: <<-SHELL
    ...
    gccFile='/opt/local/espressif/xtensa-esp32-elf/bin/xtensa-esp32-elf-gcc'
  ...
  config.vm.provision 'shell', privileged: false, env: {'PIP_SIMPLE' => options['pip_simple']}, inline: <<-SHELL
    if [ -z $PIP_SIMPLE ]; then
      python -m pip install -q --user -r /vagrant/esp-idf/requirements.txt
    else
      python -m pip install -i $PIP_SIMPLE -q --user -r /vagrant/esp-idf/requirements.txt
    fi
    grep -q 'IDF_PATH' /home/vagrant/.profile || echo "export IDF_PATH=\"/vagrant/esp-idf\"\nexport PATH=\"/opt/local/espressif/xtensa-esp32-elf/bin:/vagrant/esp-idf/tools:${PATH}\"\ncd /vagrant" >> /home/vagrant/.profile
  SHELL
end
